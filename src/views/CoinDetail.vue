    ...
    <div class="my-10 sm:mt-0 flex flex-col justify-center text-center">
      ...
      <div class="flex flex-row my-5">
        <label class="w-full" for="convertValue">
          <input v-model="convertValue" id="convertValue" type="number" class="text-center bg-whitw focus:outline-none focus:shadow-outline border border-gary-300 rounded-lg py-2 px-4 block w-full appearce-none leading-normal"
          :placeholder="`Valor en ${fromUsd ? 'USD': asset.symbol}` ">
        </label>
      </div>
      <span class="text-cl"> {{ convertResult }} {{ fromUsd? asset.symbol : 'USD'}} </span>
    </div>   
  </div>
  <line-chart 
      class="my-10" 
...
export default {
  ...
  methods:{
    ...
    getWebsite(exchange){
      return getExchanges(exchange.exchangeId)
      .then(res =>{
    ...
    async getCoin(){ 
        ...
        this.asset=await getAsset(id)  
    },
    async getHistory(){
      const id=this.$route.params.id;
      this.history=await getAssetHistory(id);
      this.isLoading=false
    },
    async getMarket(){
      const id=this.$route.params.id;
      this.markets=await getMarkets(id)
    }
  }
}
</script>
